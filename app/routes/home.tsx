...
const Component = async () => {
  ...
  if (firstYard) {
    ...
    if (elementIds.length > 0) {
      ...
      activePlantingCount = allPlantings.filter((p) => p.status !== "done").length;
    }
  }
  ...
  return (
    <main className="mx-auto max-w-6xl px-6 py-8">
      <div className="mb-8">
        ...
        <p className="text-sm text-gray-500 mt-1">Your garden at a glance.</p>
      </div>
      ...
      {!userSettings?.zone ? (
        <div className="bg-white rounded-xl border border-earth-200 shadow-sm p-8 text-center">
          ...
          <h2 className="text-lg font-semibold text-gray-900 mb-2">Welcome to Backyard Garden</h2>
        ...
        <>
          ...
          <div className="flex flex-wrap items-center gap-3 mb-6">
            <div className="inline-flex items-center gap-1.5 bg-garden-50 text-garden-700 rounded-full px-3 py-1.5 text-sm font-medium">
              <svg
            ...
            <div className="inline-flex items-center gap-1.5 bg-earth-100 text-earth-700 rounded-full px-3 py-1.5 text-sm font-medium">
              {activePlantingCount} active planting{activePlantingCount !== 1 ? "s" : ""}
            </div>
            {thisWeekTasks.length > 0 && (
              <div className="inline-flex items-center gap-1.5 bg-amber-50 text-amber-700 rounded-full px-3 py-1.5 text-sm font-medium">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="lg:col-span-2 space-y-6">
              ...
              <div className="bg-white rounded-xl border border-earth-200 shadow-sm p-5">
                <h2 className="text-base font-semibold text-gray-900 mb-4">This Week</h2>
                {thisWeekTasks.length > 0 ? (
              ...
              <div className="bg-white rounded-xl border border-earth-200 shadow-sm p-5">
                <div className="flex items-center justify-between mb-4">
                  ...
                  <Link
                    to="/log"
                    className="text-xs font-medium text-garden-600 hover:text-garden-700 no-underline"
                  >
                    View all
                  </Link>
                </div>
                        ...
                        <div key={entry.id} className="flex items-start gap-3">
                          ...
                          <div className="flex-1 min-w-0">
                            <p className="text-sm text-gray-900">
                              ...
                              {entry.plantName && (
                                <span className="text-gray-500"> &mdash; {entry.plantName}</span>
                              )}
                            </p>
                            {entry.content && (
            ...
            <div className="space-y-6">
              ...
              {firstYard ? (
                ...
                >
                  <div className="flex items-center justify-between px-5 py-3 border-b border-earth-100">
                    <div>
                      <h3 className="text-sm font-semibold text-gray-900">{firstYard.name}</h3>
                      <p className="text-xs text-gray-500">
                        {firstYard.widthFt}x{firstYard.heightFt}ft &middot; {yardElementList.length}{" "}
                        beds &middot; {plantingCount} plantings
                      </p>
                    </div>
                    <span className="text-xs font-medium text-garden-700 bg-garden-50 px-2 py-1 rounded-lg">
                      Open
                    </span>
                  </div>
                  <div className="p-4">
                ...
                >
                  <div className="mx-auto w-10 h-10 rounded-full bg-garden-50 flex items-center justify-center mb-3">
                    <svg
                  ...
                  <p className="text-xs text-gray-500 mt-1">Set up your yard and start planning.</p>
                </Link>
              )}
              ...
              {!hasRecentLogs && plantingCount > 0 && (
                <Link
                  to="/log"
                  className="group flex items-center gap-3 bg-amber-50 rounded-xl border border-amber-200 p-4 hover:bg-amber-100 transition-colors no-underline"
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-medium text-amber-900">Time to log!</p>
                    <p className="text-xs text-amber-700 mt-0.5">No entries this week.</p>
                  </div>
                </Link>
              )}
            </div>
