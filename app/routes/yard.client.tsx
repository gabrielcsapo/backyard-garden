...
export function YardEditor({
  ...
  React.useEffect(() => {
    function handleMouseMove(e: MouseEvent) {
      ...
      const newX = Math.max(0, Math.min(yard.widthFt - el.width, Math.round(drag.startElX + dx / CELL_SIZE)))
      const newY = Math.max(0, Math.min(yard.heightFt - el.height, Math.round(drag.startElY + dy / CELL_SIZE)))
    ...
    function handleMouseUp() {
      ...
      if (drag.moved) {
        ...
        setSelectedId((prev) =>
          prev === drag.elementId ? null : drag.elementId,
        )
        setActiveTool(null)
      }
    }
          ...
          <div className="pointer-events-auto space-y-3 max-h-full overflow-y-auto">
            ...
            {(SHAPE_CONFIG[selected.shapeType as ShapeType]?.plantable ?? false) && (
              <BedPlantingsPanel
                ...
                onUpdatePlanting={async (plantingId, updates: { status?: string; quantity?: number; notes?: string }) => {
                  const formData = new FormData()
  ...
  return (
    <div className="bg-white/95 backdrop-blur-sm rounded-xl border border-earth-200 shadow-lg p-4 space-y-4">
      ...
      <div>
        ...
        <div className="flex items-center gap-1">
          <button
            ...
            onClick={() => onUpdate({ rotation: String(((element.rotation ?? 0) - 45 + 360) % 360) })}
      ...
      <div className="text-xs text-gray-400 space-y-1">
        ...
        <p>Size: {element.width} x {element.height} ft</p>
...
function BedPlantingsPanel({
  ...
  onUpdatePlanting: (plantingId: number, updates: { status?: string; quantity?: number; notes?: string }) => Promise<void>
  ...
  const capacityPercent = bedAreaSqIn > 0 ? Math.min(100, Math.round((usedSqIn / bedAreaSqIn) * 100)) : 0
      ...
      <div>
        <div className="flex items-center justify-between text-xs mb-1">
          ...
          <span className="font-medium" style={{
            color: capacityPercent > 90 ? '#ef4444' : capacityPercent > 70 ? '#f59e0b' : '#22c55e',
          }}>
            {capacityPercent}%
          </span>
        </div>
        <div className="h-1.5 bg-earth-100 rounded-full overflow-hidden">
          <div
            ...
            style={{
              ...
              backgroundColor: capacityPercent > 90 ? '#ef4444' : capacityPercent > 70 ? '#f59e0b' : '#22c55e',
            }}
          />
        </div>
      </div>
                ...
                return (
                  ...
                  >
                    ...
                    <div className="text-right shrink-0">
                      <span className="text-xs font-medium text-garden-700">
                        Fits {fits}
                      </span>
                    </div>
                  </button>
...
function EditPlantingModal({
  ...
  const currentColors = STATUS_COLORS[status] ?? STATUS_COLORS.planned
            ...
            {STATUS_ORDER.map((s) => {
              ...
              return (
                <button
                  ...
                  onClick={() => setStatus(s)}
                >
                  <span className={`w-1.5 h-1.5 rounded-full ${isActive ? colors.dot : 'bg-gray-300'}`} />
                  {STATUS_LABELS[s]}
                </button>
              )
            })}
