...
export async function addPlant(formData: FormData) {
  ...
  db.insert(plants)
    .values({
      ...
      daysToHarvest: formData.get('daysToHarvest')
        ? Number(formData.get('daysToHarvest'))
        : null,
      spacingInches: formData.get('spacingInches')
        ? Number(formData.get('spacingInches'))
        : null,
      indoorStartWeeksBeforeFrost: formData.get(
        'indoorStartWeeksBeforeFrost'
      )
