...
export function getPlantingWindows(
  ...
  if (plant.indoorStartWeeksBeforeFrost != null) {
    windows.indoorStart = weeksFromDate(
      lastFrostDate,
      -plant.indoorStartWeeksBeforeFrost,
    )
  }

  if (plant.directSowWeeksBeforeFrost != null) {
    windows.directSow = weeksFromDate(
      lastFrostDate,
      -plant.directSowWeeksBeforeFrost,
    )
  }

  if (plant.transplantWeeksAfterFrost != null) {
    windows.transplant = weeksFromDate(
      lastFrostDate,
      plant.transplantWeeksAfterFrost,
    )
  }

  if (plant.daysToHarvest != null) {
    const plantDate =
      windows.transplant ?? windows.directSow ?? windows.indoorStart
    if (plantDate) {
...
export function getCurrentSeason(
  ...
  if (weeksBeforeLastFrost > 0 && weeksBeforeLastFrost <= 12)
    return 'early_spring'
  if (weeksAfterLastFrost >= 0 && weeksAfterLastFrost <= 8)
    return 'late_spring'
