...
export function checkCompanionConflicts(
  ...
  for (const existing of existingPlants) {
    ...
    if (
      newIncompat.some(
        (n) => n.toLowerCase() === existing.name.toLowerCase(),
      ) ||
      existingIncompat.some(
        (n) => n.toLowerCase() === newPlant.name.toLowerCase(),
      )
    ) {
...
export function getCompanionSuggestions(
  ...
  for (const candidate of allPlants) {
    ...
    const { conflicts, goodCompanions } = checkCompanionConflicts(
      candidate,
      existingPlants,
    )
