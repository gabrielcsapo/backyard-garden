export function getSuccessionWindows(
  ...
  if (!plant.successionIntervalWeeks || plant.directSowWeeksBeforeFrost == null) return [];

  const frost = new Date(lastFrostDate);
  const sowStart = new Date(frost);
  sowStart.setDate(sowStart.getDate() - plant.directSowWeeksBeforeFrost * 7);
...
export function shouldSowAgain(
  ...
  const now = new Date();
  const daysSinceLast = (now.getTime() - lastPlanted.getTime()) / (1000 * 60 * 60 * 24);
