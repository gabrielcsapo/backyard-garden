...
export function scoreBedForPlant(
  ...
  const sunMatch =
    !plantToAdd.sunRequirement ||
    !bed.sunExposure ||
    plantToAdd.sunRequirement === bed.sunExposure
  ...
  const spacingSqFt = plantToAdd.spacingInches
    ? (plantToAdd.spacingInches / 12) ** 2
    : 1
  const maxPlants = Math.floor(area / spacingSqFt)
  const currentPlants = plantingsInBed.reduce(
...
export function rankBedsForPlant(
  ...
  return elements
    ...
    .map((bed) => {
      const plantingsInBed = allPlantings.filter(
        (p) => p.yardElementId === bed.id,
      )
